...
Target runtime:
- The code is sent with `there run` into an already-running Kivy application.
...
- Do not raise an error because `root` is missing in normal generated
  PythonHere UI code. The executed program should assume the PythonHere execution
...
State rules:
- Code runs in a persistent remote execution namespace.
- For stateful resources that should survive across `there` commands, prefer clear global variables with obvious names.
- Reuse existing global resources when they already exist.
- Do not recreate expensive or stateful objects on every `there run` invocation unless explicitly requested.
- Keep important objects inspectable from later `there` commands.
...
- Do not rely on `print(...)` inside Kivy, Android, BLE, permission, sensor, or
  ...
  may run after `there run` output capture has ended, or may only appear in app
  logs.
...
- Keep a global reference to background worker state, for example
  `gallery_worker_thread` or `scan_thread`, so later `there` commands can inspect or
...
- If the requested code replaces the UI, keep the new root widget in a named
  global such as `last_ui` or a feature-specific name so later `there` commands can inspect
  `ids` and state.
