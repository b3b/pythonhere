...
- Reuse synthesizer across `there` commands when it already exists.
- Create synthesizer only when needed:
  if "synthesizer" not in globals() or synthesizer is None:
      synthesizer = Synthesizer()
- Keep synthesizer globally inspectable so later `there` commands can run:
  synthesizer.config
...
- For `there run` programs that configure MIDI, prefer small helper functions such as:
  get_synthesizer()
...
- Do not rely on `print(...)` inside scheduled callbacks as the only output.
  Scheduled callbacks may run after `there run` output capture has ended. Store
...
- `synthesizer.config` may contain `bytes`, so do not return it directly from
  JSON-mode `get`. For readable inspection, use:
  `there --json get "repr(synthesizer.config)"`.
  Build an explicit JSON-compatible summary when individual configuration
  fields must remain structured.
...
- If the user asks to play a note, run a minimal self-contained program that imports midistream, initializes or reuses synthesizer, plays the note, schedules note-off with Clock.schedule_once, and prints diagnostics.
- If the user asks to play a melody, run a program that schedules all notes with Clock.schedule_once and provides all_sound_off cleanup.
