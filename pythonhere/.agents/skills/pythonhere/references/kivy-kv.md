...
For PythonHere programs executed with `there run`, do not put Python callbacks
in KV.
...
Avoid referencing live-namespace globals from KV. Kivy Builder may not have
...
KV root rule:
- In PythonHere programs executed with `there run`, `KV` must end with a concrete
