...
- Reuse the shared dispatcher across cells:
...
- Keep the dispatcher globally inspectable so later cells can run:
    ble.adapter
    ble.bonded_devices
    ble.gatt
    ble.name
...
- Do not rely on `print(...)` inside BLE callbacks as the only output. BLE
  callbacks may run after notebook output capture has ended. Store every event,
...
- In BLE callbacks, prefer appending a compact tuple/dict to `ble_events` or a
  ...
  synchronous code that runs before the cell returns.
- Do not schedule delayed functions whose purpose is to print scan summaries.
  A delayed `Clock.schedule_once(...)` callback may run after notebook output
...
- Store Java objects by address or UUID-like key so later cells can use them.
- Do not print huge advertisement dumps or service trees directly.
- Print compact immediate summaries from the initiating cell and store full
...
- Do not schedule delayed print summaries after scans. Print only one immediate
  line before the cell returns, naming the globals where results will appear.
...
- If the user asks to scan, generate a minimal Able scan cell with a shared dispatcher, event handlers, `ble_devices_by_address`, and a scheduled stop.
